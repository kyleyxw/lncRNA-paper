...
results  <- c()
for (i in 4001:length(genes)){ 
  print(paste("No",i,"gene is running"))
  tmp <- ncRNA(chr=chr,gene_name = genes[i],genofile = genofile,obj_nullmodel = obj_nullmodel,geno_missing_imputation="mean")
...
save(results, file = paste0("/rprojectnb2/pelosolab/yuxuan/lncRNA/",lipid,"/topmed_lncRNA_",annot,"_", lipid,"_",chr, "part3.Rdata"))
