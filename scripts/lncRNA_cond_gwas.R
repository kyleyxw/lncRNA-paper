...
# ncRNAs by annot
lncrna <- read.table("region_list_sort.bed",header = F)
...
## GWAS to cond
topmed_sig_lncrna <- read_csv("topmed_sig_lncrna.csv")
...
for (i in 1:nrow(lncrna)){
  ...
  # genofile
  gdsfile <- paste0("topmed/frz8_agds/freeze.8.",lncrna$chr[i],".pass_and_fail.gtonly.minDP0.gds")
  genofile <- seqOpen(gdsfile)
  
  # null model
  obj_nullmodel <- readRDS(paste0("lncRNA/null_model/",lncrna$lipid[i], ".rds"))
  
  phenotype.id <- as.character(obj_nullmodel$id_include)
  
  # known_loci
  
  known_loci <- get(load("lipid_known_loci_info.Rdata"))
...
write.csv(results, file = paste0("lncrna_gwas_500kb_cond.csv"))
